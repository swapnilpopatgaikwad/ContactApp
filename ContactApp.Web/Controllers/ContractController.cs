...
using ContactApp.DataAccess.Migrations;
...
namespace ContactApp.Web.Controllers
{
    ...
    public class ContractController : Controller
    {
            ...
            else if (fileInput != null && fileInput.Length > 0)
            {
                try
                {
                    using (var stream = new StreamReader(fileInput.OpenReadStream()))
                    {
                        ...
                        var contacts = JsonConvert.DeserializeObject<List<Models.Contact>>(jsonData);
        ...
        private void SaveContactsToDatabase(List<Models.Contact> contacts)
        {
            try {
